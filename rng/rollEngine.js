...
function rollAura({ userId = null, luckBonus = 0, godlikeNoobAvailable = true } = {}) {
    ...
    if (luckBonus >= 50) minOdds = 2; // Filters out 1-in-1
    if (luckBonus >= 150) minOdds = 5; // Filters out 1-in-2, 1-in-3, 1-in-4
    ...
    if (luckBonus >= 7500) minOdds = 500;
    if (luckBonus >= 10000) minOdds = Math.floor(luckBonus / 10); // Aggressive scaling for very high luck
